...
$('.like-btn').click(function () {
	let lengProducts = $('.product__like-btn[disabled]').length;
	let likeProducts = $('.product__like-btn[disabled]').closest('.product');
	for (let i = 0; i < lengProducts; i++) {
		let dataId = likeProducts.eq(i).attr('data-id');
		likeProducts.eq(i).find('.product__btn').trigger('click');
		$('.like-content__list').find('.like-product__delete').trigger('click');
	}
})
$(document).ready(function () {
	let leng = $('.like-content__list').length;
});
// $(document).ready(function () {
